...
/**
 * The path scanning 7 policy first selects the nearest neighbours.
 * Then, among multiple nearest neighbours, it minimises the length of the historical route.
 *
 * @author gphhucarp, William Huang
 */

public class PathScanning7Policy extends AllocationPolicy {
    // a sufficiently large coefficient to guarantee the priority of nearest vehicle
    public static final double ALPHA = 10000;

    public PathScanning7Policy(PoolFilter poolFilter, TieBreaker tieBreaker) {
        super(poolFilter, tieBreaker);
        name = "\"PS7\"";
    }

    public PathScanning7Policy(TieBreaker tieBreaker) {
        this(new FeasiblePoolFilter(), tieBreaker);
    }

    public PathScanning7Policy() {
